...
GtkWindow* create_popup_window(GtkWindow *parent , gint width, gint height)
{
...
void gtk_toggle_button_mark_active(GtkToggleButton *btn, gboolean act)
{
...
/** 获取指定按钮以外的第一个激活的按钮 */
GtkToggleButton* get_active_toggle_button_except(GList *list , GtkToggleButton *btn)
{
	GtkToggleButton *b = NULL;

	for (; list; list = list->next) {
		b = GTK_TOGGLE_BUTTON(list->data);
		if (b != btn && gtk_toggle_button_get_active(b)) {
			break;
		} else {
			b = NULL;
		}
...
PangoLayout* pango_cairo_prepare_layout(cairo_t *cr, char *text, char *fontname)
{
...
{
	PangoLayout *layout = pango_cairo_prepare_layout(cr, text, fontname);
